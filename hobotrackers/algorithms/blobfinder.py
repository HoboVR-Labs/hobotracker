...
"""Blobfinder algorithm optimized to find the largest single blob in an image."""

import cv2
import numpy as np
from copy import copy

from hobotrackers.util.manual_color_mask_calibration import ColorRange


def reversed_enumerate(l):
    """Reverse and enumerate a list."""
...
def mask_for_hsv_range(arr, colormask: ColorRange):
    """
    Mask the input image to everything within the color range.

    :param arr: the input image
    :param colormask: the color range.
...
class BlobFinder(object):
    """A blobfinder that will find the largest blob in a video feed."""

    def __init__(self, color_range: ColorRange, draw_find: bool = False):
        """
        Create a blobfinder that will run on a vide feed.

        :param color_range: the range of HSV colors to mask and track.
        :param draw_find: If true, draw what the algorithm found on the video feed
        """
